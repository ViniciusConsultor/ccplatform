...
{
    /// <summary>
    /// 数据访问类 t_ChannelState ，此类请勿动，以方便表字段更改时重新生成覆盖
    /// </summary>
    public partial class t_ChannelState : DALHelper
    {
        ...
        {
            strSql = new StringBuilder();
            strSql.Append("DELETE FROM t_ChannelState");
            strSql.Append(" WHERE iChID=@in_iChID");
        ...
        {
            strSql = new StringBuilder();
            strSql.Append("SELECT COUNT(1) FROM t_ChannelState");
            strSql.Append(" WHERE iChID=@in_iChID");
        ...
        {
            strSql = new StringBuilder();
            strSql.Append("SELECT * FROM t_ChannelState");
            strSql.Append(" WHERE iChID=@in_iChID");
